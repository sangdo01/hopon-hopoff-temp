...
const AdminLayout = ({ children }: { children: ReactNode }) => {
  ...
  // Define paths where the layout should not be applied
  const pathname = '';
  const excludedPaths = ["/admin/auth/sign-in", "/admin/auth/sign-up"];
  const isExcludedPage = excludedPaths.includes(pathname);

  // If the current path is excluded, render only the children
  if (isExcludedPage) {
    return <>{children}</>;
  }
